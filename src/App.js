...
class App extends Component{
  ...
  getFilteredImages=()=>{
    ...
    const filteredImages=imagesList.filter(img=>(img.category===activeTabId))
  ...
  onClickImage=(thumbnailUrl)=>{
    ...
    const img=imagesList.filter(each=>(each.thumbnailUrl===thumbnailUrl))
    const {imageUrl}=img[0]
    if(image===imageUrl){
      const newImage=imagesList[Math.floor(Math.random()*imagesList.length)].imageUrl
  ...
  render(){
    const {score,time,activeTabId,image,isActive}=this.state
    const filteredImages=this.getFilteredImages()
    return(
      <div className="bg-container">
      <nav className="navbar">
      <img src="https://assets.ccbp.in/frontend/react-js/match-game-website-logo.png" className="logo" alt="website logo"/>
      <ul className="score-details-container">
      <li className="list-items">
      <p className="score">Score: <span className="score-num">{score}</span></p>
      </li>
      <li className="timer">
      <img src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png" className="timer-img" alt="timer"/>
      ...
      {!isActive && (
        ...
        <img src={image} className="display-img" alt="match"/>
        <ul className="tab-items-list">
        {tabsList.map(eachTab=>(
          <li key={eachTab.tabId}>
          <button className={`tab-btn ${activeTabId===eachTab.tabId ? "highlight-tab": ""}`} type="button" onClick={()=>this.updateActiveTabId(eachTab.tabId)}>{eachTab.displayText}</button>
          </li>
        ))}
        </ul> 
        ...
        {filteredImages.map(eachImg=>(
          <li key={eachImg.id}>
          <button className="img-btn" type="button" onClick={()=>this.onClickImage(eachImg.thumbnailUrl)}>
          <img src={eachImg.thumbnailUrl} alt="thumbnail" className="thumbnail-img"/>
          </button>
          </li>
        ))}
        </ul>
      ...
      {isActive && (
        ...
        <img src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png" alt="trophy" className="image"/>
        <p className="score-scored">YOUR SCORE</p>
        <p className="score-number">{score}</p>
        <button className="play-again-btn" type="button" onClick={this.playAgain}>
        <img src="https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png" alt="reset" className="reset-img"/>
        PLAY AGAIN
        </button>
        </div>
      )}
